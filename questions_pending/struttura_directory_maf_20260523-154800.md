...
type: "query-answer"
query: "come è definita la struttura di directory di un progetto MAF?"
created: "2026-05-23"
pages_read: ["concepts/factory-pattern-for-agents","entities/microsoft-agent-framework","sources/microsoft-agent-framework-agents-python-howto"]
---

# Struttura directory di un progetto MAF

La convenzione adottata nel progetto è di organizzare ogni agent in un proprio modulo sotto `afw_core/agents/` e separare le responsabilità infrastrutturI (client, tools, mcps, llms) in pacchetti distinti. Questo permette di iniettare `client`, `options` e `tools` nel factory `create_agent()`.
...
- `afw_core/mcps/` — proxy MCP per tool esterni. [[entities/microsoft-agent-framework]]

Fonte principale: [[concepts/factory-pattern-for-agents]], supportato da [[entities/microsoft-agent-framework]] e [[sources/microsoft-agent-framework-agents-python-howto]].
